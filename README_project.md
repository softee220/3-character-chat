# 환승연애 PD 친구 캐릭터 챗봇 프로젝트

> **프로젝트 설명**: 최근 '환승연애' 막내 PD가 된 내 친구가 신규 프로그램 기획을 위해 연애 스토리를 수집하고, AI 에이전트로 '미련도'를 분석해주는 대화형 챗봇
...
- **현상**: 사용자 대화와 무관한 유사 사례가 검색됨
- **원인**: 
  - 임베딩 모델이 한국어 구어체와 문맥을 제대로 이해하지 못함
  - 대화의 누적 맥락이 아닌 단일 메시지만으로 검색
- **증상**: "아직도 생각나" 같은 표현에 "완전히 정리했다"는 사례가 검색됨
...
1. ❌ 단일 메시지만으로 검색 → 정확도 낮음
2. ✅ **최근 3턴의 대화 기록을 결합하여 검색** → 정확도 향상
3. ✅ **최종 리포트 생성 시 전체 대화 기록 사용** → 가장 정확한 결과
...
def calculate_regret_index(self, user_message: str, use_rag: bool = True) -> Dict[str, float]:
    ...
    if use_rag and self.rag_service:
        # 전체 대화 맥락 사용
        full_context = self._build_full_context(user_message)
        similar_cases = self.rag_service.search_similar_cases(full_context, top_k=3)
        # LLM 정규화 수행
        normalized = self._normalize_with_llm(user_message, initial_scores, similar_cases)
```
...
### 문제 6 해결: Docker Volume 설정

**해결 방법**: `docker-compose.yml`에 ChromaDB 데이터 디렉토리 마운트 추가
...
- **역할 분담**: 프로듀서-엔지니어 간 명확한 업무 분담으로 효율성 증가
  - 프로듀서: 캐릭터 설정, 질문 설계, 데이터 준비
  - 엔지니어: 기술 구현, 성능 최적화, 배포
...
- **바이브코딩**: AI와 함께 코드를 작성하며 학습하는 방식이 매우 효과적
  - AI가 제안한 코드를 이해하고 개선하는 과정에서 깊이 있는 학습 가능
- **프롬프트 엔지니어링**: LLM에게 명확한 지시를 내리는 것이 얼마나 중요한지 체감
  - 작은 프롬프트 변경이 결과에 큰 영향을 미침
- **AI의 한계**: AI가 모든 것을 해결해주지는 않음
  - 비즈니스 로직과 도메인 지식은 여전히 사람이 설계해야 함
  - AI는 도구일 뿐, 최종 결정은 사람이 내려야 함
...
- **배포 URL**: (배포 완료 후 업데이트 예정)
- **배포 방법**: Vercel CLI를 통한 배포 예정
